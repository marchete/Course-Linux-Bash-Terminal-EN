...
checkHistory "cd .."
sendMessage "<h1>You should be on folder /project now, try to move to the root folder, user the command: <div class=\"well well-sm\">cd /</div></h1></p>In this case the cd command will use an absolute path."
checkHistory "cd /"
sendMessage "<h1>Great! What will be in that folder? Let's find out with:<div class=\"well well-sm\">ls -l -h</div></h1></p>ls command is like dir on Windows, it shows files and directories. The -l parameter is for list one per line, and -h is for human readable sizes. If you don't specify a path, ls will use the current directory"
checkHistory "ls -l -h"
sendMessage "<h1>A lot of folders! You can also use a path with ls to show the contents of any folder in the system:<div class=\"well well-sm\">ls /usr/lib</div></h1></p>ls is used with an absolute path, without parameters, notice how different is the output."
checkHistory "ls /usr/lib"
sendMessage "<h1>Sometimes names are hard to write. Let's use Tab to autocomplete, write:<div class=\"well well-sm\">ls -l -h /lib/x86</div>and press Tab, you'll see how the system autocompletes the name.</h1></p>Tab autocompletion is a must have for a greater lazyness, it simplifies a lot the completion of commands."
checkHistory "ls -l -h /lib/x86_64-linux-gnu/"
sendMessage "<h1>Other lazy commands are up and down cursor to check command history. OK, now let's use the command:<div class=\"well well-sm\">df -h</div></h1></p>df command shows the free space on all filesystems, and information "
checkHistory "df -h"
